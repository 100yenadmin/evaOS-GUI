...
const DEPLOYED_NEGATIVE_PROBE_REQUEST_FIELDS = ['proofMode', 'expectedCandidate'] as const;
...
type DeployedNegativeProbeRequest = {
  proofMode: typeof DEPLOYED_NEGATIVE_PROBE_MODE;
  expectedCandidate: PublicCandidate;
};

type RuntimeReceiptRequest =
  | { kind: 'receipt'; value: PublicRequest }
  | { kind: 'deployed-negative-probe'; value: DeployedNegativeProbeRequest };
...
type PluginHttpApi = {
  registerHttpRoute(route: {
    ...
    auth: 'gateway';
    match: 'exact';
    handler: (request: RequestLike, response: ResponseLike) => Promise<boolean>;
  }): void;
};

/** Register the one exact, gateway-authenticated Mac-control receipt route. */
export function registerMacControlRuntimeReceiptRoute(api: PluginHttpApi): void {
...
export function createMacControlRuntimeReceiptHandler(options: RuntimeReceiptOptions = {}) {
  ...
  return async (request: RequestLike, response: ResponseLike): Promise<boolean> => {
    ...
    let connectorResponse: FetchResponseLike | undefined;
    let rawConnectorResponse: string;
...
async function readRuntimeReceiptRequest(
  request: RequestLike
): Promise<{ ok: true; value: RuntimeReceiptRequest } | { ok: false; status: number }> {
  ...
  return {
    ...
    value: {
      ...
      value: { proofMode: DEPLOYED_NEGATIVE_PROBE_MODE, expectedCandidate: expectedCandidate as PublicCandidate },
    },
  };
}

function candidateMatchesVerifier(candidate: PublicCandidate, verifier: ReceiptVerifierConfig): boolean {
  return (
...
function createDeployedNegativeProbeProof(
...
function verifyAuthority(
  ...
  const context = parsed as ExecutionContext;
  if (
...
async function readBoundedConnectorResponse(response: FetchResponseLike): Promise<string> {
  if (response.body) {
    const chunks: ByteBuffer[] = [];
    let total = 0;
...
function validOpaqueId(value: unknown, maxLength: number): value is string {
...
function decodeBase64Url(value: string): ByteBuffer | undefined {
...
function decodeCanonicalBase64(value: string): ByteBuffer | undefined {
...
function validSshSignature(value: string): boolean {
...
function claimExecutionContext(
  cache: Map<string, number>,
  context: ExecutionContext,
  nowMs: number
): { ok: true } | { ok: false; status: 409; code: 'execution_context_replayed' } {
...
function pruneReplayCache(cache: Map<string, number>, nowMs: number): void {
...
function sendError(response: ResponseLike, status: number, code: string): void {
