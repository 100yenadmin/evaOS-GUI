...
function inspectMacosZipBridgePayload(zipPath) {
  const script = [
    ...
    '        result["pythonLauncherValid"] = (stat.S_ISLNK(launcher_mode) and launcher_bytes == b"python3.12") or (stat.S_ISREG(launcher_mode) and bool(stat.S_IMODE(launcher_mode) & 0o111) and macho_has_arch(launcher_bytes, expected_python_arch))',
    ...
    '        actual_python = {suffix[len("python/"):]: info for suffix, info in entries.items() if suffix.startswith("python/")}',
    '        inventory_valid = metadata_valid and len(declared_paths) == len(declared_entries) and declared_paths == sorted(declared_paths) and len(declared_paths) == len(set(declared_paths)) and set(declared_paths) == set(actual_python)',
    '        if inventory_valid:',
    ...
    '                if declared.get("mode") != stat.S_IMODE(mode):',
    '                    inventory_valid = False',
    '                    break',
    '                if declared_type == "file":',
    '                    declared_sha256 = declared.get("sha256")',
    '                    declared_size = declared.get("size")',
    '                    if not stat.S_ISREG(mode) or not isinstance(declared_size, int) or declared_size < 0 or not isinstance(declared_sha256, str) or len(declared_sha256) != 64:',
    '                        inventory_valid = False',
    '                        break',
