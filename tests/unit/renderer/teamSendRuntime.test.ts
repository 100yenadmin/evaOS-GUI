...
describe('buildTeamSendRuntime', () => {
  ...
  it('locks teammate sendbox immediately when the active run ack targets that teammate', () => {
    const runtime = buildTeamSendRuntime({
      slot_id: 'worker',
      isLeader: false,
      runView: {
  ...
  it('resolves teammate stop target from the active run ack before child turn state arrives', () => {
    expect(
      resolveTeamStopTarget(
        {
          activeRun: {
