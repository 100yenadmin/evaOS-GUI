...
async function main() {
  await Promise.all([
    ...
    esbuild.build({
      ...SHARED_OPTIONS,
      entryPoints: [path.join(ROOT, 'packages/desktop/src/process/resources/builtinMcp/evaosMacControlServer.ts')],
      outfile: path.join(ROOT, 'out/main/builtin-mcp-evaos-mac-control.js'),
    }),
  ]);
}
