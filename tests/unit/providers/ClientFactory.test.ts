...
describe('ClientFactory', () => {
  ...
  describe('createRotatingClient', () => {
    ...
    it('creates GeminiRotatingClient for USE_GEMINI', async () => {
      ...
      const calls = vi.mocked(GeminiRotatingClient).mock.calls;
    ...
    it('normalizes base URL for new-api platform', async () => {
      ...
      const calls = vi.mocked(OpenAIRotatingClient).mock.calls;
    ...
    it('uses evaOS Workbench metadata headers for OpenAI-compatible requests', async () => {
      await ClientFactory.createRotatingClient(mockProvider);
      const calls = vi.mocked(OpenAIRotatingClient).mock.calls;
      const config = calls[0][1];
    ...
    it('does not use AionUi metadata headers for fallback OpenAI-compatible requests', async () => {
      ...
      expect(config.defaultHeaders).not.toMatchObject({
    ...
    it('passes model to Anthropic config', async () => {
      ...
      const calls = vi.mocked(AnthropicRotatingClient).mock.calls;
      const config = calls[0][1];
