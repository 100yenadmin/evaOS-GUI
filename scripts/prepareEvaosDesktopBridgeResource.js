...
const { getTruthyReleaseFlags } = require('./packagingProfile');

const projectRoot = path.resolve(__dirname, '..');
const defaultBridgeResourceDir = path.join(projectRoot, 'resources', 'Bridge');
const defaultBridgeSourceCacheDir = path.join(projectRoot, '.cache', 'evaos-desktop-bridge-source');
const bridgeResourceDir = path.resolve(process.env.EVAOS_DESKTOP_BRIDGE_RESOURCE_DIR || defaultBridgeResourceDir);
const bridgeSourceCacheDir = path.resolve(process.env.EVAOS_DESKTOP_BRIDGE_CACHE_DIR || defaultBridgeSourceCacheDir);
const defaultBridgeSourceRepo = 'https://github.com/electricsheephq/evaos-desktop-bridge.git';
/** Default immutable evaos-desktop-bridge source commit used when callers do not provide EVAOS_DESKTOP_BRIDGE_SOURCE_REF. */
const defaultBridgeSourceRef = '207f6528461ecae51c39efd2654733c1b07d39a4';
...
/** Returns true when the current package profile/env must refuse diagnostic placeholder Bridge resources. */
function shouldRejectPlaceholder() {
  return shouldRequireRealBridge() || process.env.EVAOS_PACKAGING_PROFILE === 'functional-smoke';
}

function shouldRequirePinnedSourceRef() {
  return shouldRequireRealBridge() || process.env.EVAOS_PACKAGING_PROFILE === 'functional-smoke';
}
...
/** Returns true for source refs that are not full commit SHAs and therefore invalid for proof/release lanes. */
...
function getGitCommit(cwd, ref) {
  return gitValue(cwd, ['rev-parse', '--verify', `${ref}^{commit}`]);
}

function assertPinnedSourceDir(sourceDir) {
  if (!shouldRequirePinnedSourceRef()) return;

  const ref = selectedBridgeSourceRef();
  if (isMutableBridgeSourceRef(ref)) {
    throw new Error(
      [
        'Workbench Bridge resource proof requires a pinned evaos-desktop-bridge source ref.',
        'Set EVAOS_DESKTOP_BRIDGE_SOURCE_REF to an approved full 40-character commit SHA, not a branch, tag, or HEAD.',
      ].join(' ')
    );
  }

  const sourceCommit = gitValue(sourceDir, ['rev-parse', 'HEAD']);
  const requestedCommit = getGitCommit(sourceDir, ref);
  if (!sourceCommit || !requestedCommit || sourceCommit !== requestedCommit) {
    throw new Error(
      [
        `Pinned evaos-desktop-bridge ref ${ref} does not match local source checkout ${sourceDir}.`,
        `Local HEAD: ${sourceCommit || 'unknown'}.`,
        `Requested ref resolves locally to: ${requestedCommit || 'unknown'}.`,
        'Use EVAOS_DESKTOP_BRIDGE_SOURCE_REF with a matching checkout or allow the build to fetch the pinned ref.',
      ].join(' ')
    );
  }
}

/** Returns local Bridge source checkout candidates allowed by the current env before a pinned fetch. */
function sourceCandidates() {
  if (process.env.EVAOS_DESKTOP_BRIDGE_SOURCE_DIR) {
    return [process.env.EVAOS_DESKTOP_BRIDGE_SOURCE_DIR];
  }
  if (truthy(process.env.EVAOS_DESKTOP_BRIDGE_DISABLE_DEFAULT_CANDIDATES)) {
    return [];
  }
  ...
  return [
    ...
    '/Volumes/LEXAR/repos/gitnexus-approved/evaos-desktop-bridge',
  ].filter(Boolean);
}

/** Resolves a Bridge source tree, validating pinned refs for functional-smoke and release-like builds. */
function resolveBridgeSourceDir() {
  for (const candidate of sourceCandidates()) {
    ...
    if (fs.existsSync(path.join(sourceDir, 'src', 'evaos_desktop_bridge', 'cli.py'))) {
      assertPinnedSourceDir(sourceDir);
      return sourceDir;
    }
  }
...
function prepareBridgeSourceCheckout() {
  ...
  const ref = selectedBridgeSourceRef();
  if (shouldRequirePinnedSourceRef() && isMutableBridgeSourceRef(ref)) {
    throw new Error(
      [
        'Workbench Bridge resource proof requires a pinned evaos-desktop-bridge source ref.',
        'Set EVAOS_DESKTOP_BRIDGE_SOURCE_REF to an approved full 40-character commit SHA, not a branch, tag, or HEAD.',
  ...
  if (fs.existsSync(path.join(bridgeSourceCacheDir, 'src', 'evaos_desktop_bridge', 'cli.py'))) {
    assertPinnedSourceDir(bridgeSourceCacheDir);
...
function resetBridgeSourceOrigin(repo) {
  if (!fs.existsSync(path.join(bridgeSourceCacheDir, '.git'))) return;
  runGit(['remote', 'set-url', 'origin', repo], bridgeSourceCacheDir, repo);
}
...
/** Redacts tokenized GitHub clone URLs from command output before logs or thrown errors are surfaced. */
...
/** Checks executable permission and Mach-O magic for a local helper binary. */
...
/** Returns the packaged CLI wrapper script that runs the bundled Bridge source through Python. */
...
function sourcePathForManifest(sourceDir) {
  if (!sourceDir || sourceDir === PLACEHOLDER_SOURCE) return PLACEHOLDER_SOURCE;
  if (path.resolve(sourceDir) === bridgeSourceCacheDir) return 'managed-cache/evaos-desktop-bridge';
  return `local-checkout/${path.basename(sourceDir) || 'evaos-desktop-bridge'}`;
}

function preparePlaceholderBridgeResource(error) {
  if (shouldRejectPlaceholder() || !shouldAllowPlaceholder()) {
    throw error;
  }
  ...
  writePeekabooFallbackWrapper(path.join(bridgeResourceDir, 'bin'));
  writeConnectorHelperWrapper(path.join(bridgeResourceDir, 'bin'));
  writeManifest({
...
function main() {
  ...
  writeManifest({
    schema: 'evaos-desktop-bridge-resource/v1',
    requestedSourceRef: selectedBridgeSourceRef(),
    sourcePath: sourcePathForManifest(bridgeSourceDir),
...
module.exports = {
  bridgeWrapperScript,
  defaultBridgeSourceRef,
  isMachOExecutable,
  isMutableBridgeSourceRef,
  resolveBridgeSourceDir,
  sanitizeCommandText,
  shouldRejectPlaceholder,
  sourceCandidates,
};
