...
import { buildTeamSendRuntime, resolveTeamStopTarget } from './teamSendRuntime';
  ...
  const teamRuntime =
    team_id && slot_id
      ? buildTeamSendRuntime({
          ...
          runView: teamRunView,
          onStop: async () => {
            const stopTarget = resolveTeamStopTarget(teamRunView, slot_id, Boolean(isLeader));
            if (!stopTarget) return;
            if (stopTarget.kind === 'team-run') {
              await ipcBridge.team.cancelRun.invoke({
                team_id,
                team_run_id: stopTarget.team_run_id,
                target_slot_id: stopTarget.target_slot_id,
              });
              return;
            }
