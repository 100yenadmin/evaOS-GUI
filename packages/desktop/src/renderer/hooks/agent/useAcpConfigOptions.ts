...
export function getOptionCurrentValue(option: AcpConfigOptionDto | null | undefined): string | null {
  return option?.current_value ?? option?.selected_value ?? null;
}
...
export function deriveSelectOption(
  ...
  if (!option || (option.option_type ?? option.type) !== 'select' || !Array.isArray(option.options)) return null;
  return {
    id: option.id,
    category,
...
export function hasObservedValue(
  ...
  if (response.confirmation !== 'observed' || !Array.isArray(response.config_options)) return false;
...
export function useAcpConfigOptions({
  ...
  const {
    ...
    mutate,
    isLoading,
  } = useSWR<AcpConfigOptionDto[] | null>(enabled && conversation_id ? key : null, fetchConfigOptions, {
  ...
  const reload = useCallback(async () => {
    if (!enabled || !conversation_id) return null;
    await prepareRuntime?.();
    const next = await fetchConfigOptions(key);
    replaceSnapshot(next);
    return next;
  }, [conversation_id, enabled, key, prepareRuntime, replaceSnapshot]);
