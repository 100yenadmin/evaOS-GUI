...
describe('collectBackendInstallDiagnostics', () => {
  ...
  it('records no-acp managed-resource bundle state as install-needed diagnostics', () => {
    const files = new Map<string, { mtimeMs: number; size: number; content?: string }>([
        ...
        '/Applications/AionUi.app/Contents/Resources/bundled-aioncore/darwin-arm64/manifest.json',
        {
          ...
          content: JSON.stringify({
            ...
            resourceShape: {
              ...
              managedNodeRuntime: { present: true },
            },
          }),
        },
      ],
    ...
    const diagnostics = collectBackendInstallDiagnostics(
      ...
      },
      {
        appVersion: '2.1.10',
        arch: 'arm64',
        execPath: '/Applications/AionUi.app/Contents/MacOS/AionUi',
        isPackaged: true,
        platform: 'darwin',
        readFile: (filePath) => files.get(filePath)?.content,
        stat: (filePath) => files.get(filePath),
    ...
    expect(diagnostics).toMatchObject({
      ...
      manifestManagedNodeRuntimePresent: true,
      manifestManagedResourcesBundle: 'no-acp',
      manifestManagedResourcesPresent: false,
