...
CUA_DRIVER_BIN_CANDIDATES = (
    "cua-driver",
    "/opt/homebrew/bin/cua-driver",
    "/usr/local/bin/cua-driver",
)
MAC_CONTROL_ENGINE_ORDER = (
    "cua_driver",
    "peekaboo",
    "post_to_pid_helper",
    "accessibility",
...
class CuaDriverClient(Protocol):
    def call(self, tool: str, arguments: dict[str, object], *, timeout: float = 10.0) -> CommandResult:
        ...


class CuaDriverMcpClient:
    """Tiny line-delimited JSON-RPC client for `cua-driver mcp`."""
    ...
    def call(self, tool: str, arguments: dict[str, object], *, timeout: float = 10.0) -> CommandResult:
        ...
        try:
            ...
            if proc.stdin is None or proc.stdout is None:
                raise RuntimeError("cua-driver MCP did not expose stdio pipes")
            ...
            self._send_mcp(
                proc,
                {
                    ...
                    "params": {
                        ...
                        "capabilities": {},
                        "clientInfo": {"name": "evaos-desktop-bridge", "version": "0"},
                    },
                },
            )
            initialized = self._read_mcp_response(proc, expected_id=initialize_id, deadline=deadline)
            if initialized is None:
                raise TimeoutError("cua-driver MCP did not return initialize")
            if initialized.get("error") is not None:
                return self._rpc_error_result(tool, initialized, stderr=stderr)
            self._send_mcp(proc, {"jsonrpc": "2.0", "method": "notifications/initialized", "params": {}})

            # The bridge opens a short-lived MCP process per call. CUA sessions
            # are meant for long-lived agents; passing a stable session id here
            # makes cua-driver immediately end that session and ignore the tool
            # call. Run cursor-less and let element_token/window ids carry state.
            call_arguments = dict(arguments)
            call_arguments.pop("session", None)

            tool_id = next_id()
            self._send_mcp(proc, {"jsonrpc": "2.0", "id": tool_id, "method": "tools/call", "params": {"name": tool, "arguments": call_arguments}})
            response = self._read_mcp_response(proc, expected_id=tool_id, deadline=deadline)
            if response is None:
                raise TimeoutError("cua-driver MCP did not return a tool result")

            return self._tool_response_to_command_result(tool, response, stderr=stderr)
        except TimeoutError:
            return CommandResult(
                ok=False,
                data={"tool": tool, "engine": "cua_driver"},
                errors=[make_error(code="cua_driver_timeout", message="cua-driver MCP timed out before returning a tool result.", guidance="Falling back to the next Mac-control engine.")],
                provenance={"source": "cua_driver", "tool": tool},
            )
            ...
            return CommandResult(
                ok=False,
                data={"tool": tool, "engine": "cua_driver"},
                errors=[make_error(code="cua_driver_unavailable", message="cua-driver MCP could not be started.", guidance="Install or restart CuaDriver.app; the bridge will fall back to the next engine.")],
                warnings=[str(redact_value(exc))],
                provenance={"source": "cua_driver", "tool": tool},
            )
        finally:
            if proc is not None:
                stderr = self._finish_mcp_process(proc)

    def _send_mcp(self, proc: subprocess.Popen[str], payload: dict[str, Any]) -> None:
        if proc.stdin is None:
            raise RuntimeError("cua-driver MCP stdin closed")
        proc.stdin.write(json.dumps(payload, separators=(",", ":")) + "\n")
        proc.stdin.flush()

    def _read_mcp_response(self, proc: subprocess.Popen[str], *, expected_id: int, deadline: float) -> dict[str, Any] | None:
    ...
    def _finish_mcp_process(self, proc: subprocess.Popen[str]) -> str:
        try:
    ...
    def _rpc_error_result(self, tool: str, response: dict[str, Any], *, stderr: str) -> CommandResult:
        error = response.get("error") if isinstance(response.get("error"), dict) else {}
        return CommandResult(
            ok=False,
            data={"tool": tool, "engine": "cua_driver"},
            errors=[make_error(code="cua_driver_rpc_error", message=str(error.get("message") or "cua-driver MCP returned a JSON-RPC error."), guidance="Falling back to the next Mac-control engine.")],
            warnings=[str(redact_value(stderr.strip()))] if stderr.strip() else [],
            provenance={"source": "cua_driver", "tool": tool},
        )

    def _tool_response_to_command_result(self, tool: str, response: dict[str, Any], *, stderr: str) -> CommandResult:
        if not isinstance(response, dict):
            return CommandResult(
                ok=False,
                data={"tool": tool, "engine": "cua_driver"},
                errors=[make_error(code="cua_driver_bad_response", message="cua-driver MCP returned no parseable tool response.", guidance="Falling back to the next Mac-control engine.")],
                warnings=[str(redact_value(stderr.strip()))] if stderr.strip() else [],
                provenance={"source": "cua_driver", "tool": tool},
            )
        if response.get("error") is not None:
            return self._rpc_error_result(tool, response, stderr=stderr)
        ...
        return CommandResult(
            ...
            data={
                ...
                "structured": redact_value(structured),
                "engine": "cua_driver",
            },
            errors=[] if not is_error else [make_error(code="cua_driver_tool_failed", message=text or "cua-driver tool failed.", guidance="Falling back to the next Mac-control engine.")],
            warnings=[str(redact_value(stderr.strip()))] if stderr.strip() else [],
            provenance={"source": "cua_driver", "tool": tool},
        )
...
""".strip()
    ...
    def __init__(
        ...
        *,
        ...
        cua_driver_client: CuaDriverClient | None = None,
    ) -> None:
        self.runner = runner
        ...
        self.cua_driver_client = cua_driver_client
        self._engine_status_cache: dict[str, tuple[float, dict[str, Any]]] = {}

    def status(self) -> CommandResult:
        frontmost = self._frontmost_app()
        mirroring_status = self.iphone_mirroring_status()
        screen_sharing = self.screen_sharing_status()
        control_engines = self._control_engine_status()
        return CommandResult(
            ok=True,
            data={
                "proof_scope": {
    ...
    def control_status(self) -> CommandResult:
        ...
        return CommandResult(
            ...
            data={
                ...
                "peekaboo": control_engines["peekaboo"],
                "control_engines": control_engines,
                "permissions": {
                    "accessibility": self._permission_status("accessibility"),
                    "screen_recording": self._permission_status("screen_recording"),
    ...
    def desktop_see(self, *, max_chars: int = 4000, max_nodes: int = 200) -> CommandResult:
        ...
        warnings: list[str] = []
        client = self._cua_driver_action_client()
        if client is not None:
            cua_seen = self._cua_driver_see(client=client, max_nodes=max_nodes)
            if cua_seen.ok:
                return cua_seen
            warnings.extend(cua_seen.warnings)
            warnings.extend(str(error.get("message") or error.get("code") or "cua-driver see failed") for error in cua_seen.errors)
        peekaboo = self._peekaboo_status()
    ...
    def desktop_click(
        ...
        *,
        ...
        if sensitive_block is not None:
            return sensitive_block
        cua_warnings: list[str] = []
        if resolved_engine == "cua_driver":
            client = self._cua_driver_action_client()
            cua_target = self._cua_driver_target_from_resolved(resolved)
            if client is not None and cua_target is not None:
                cua_args = {**cua_target, "delivery_mode": "background", "session": "evaos-desktop-bridge"}
                cua_result = client.call("click", cua_args, timeout=15.0)
                acceptable, reason = self._cua_driver_result_acceptable(cua_result, tool="click")
                if acceptable:
                    return CommandResult(
                        ok=True,
                        data={
                            ...
                            "point": self._point(x, y),
                            "engine": "cua_driver",
                            "cua": cua_result.data.get("structured") if isinstance(cua_result.data.get("structured"), dict) else {},
                        },
                        warnings=cua_result.warnings,
                        provenance={"source": "cua_driver"},
                    )
                cua_warnings.extend(self._cua_driver_warning(reason or "cua-driver click failed"))
        if resolved_ax_target is not None:
            ...
            if "AXPress" in {str(item) for item in resolved_actions}:
                ...
                pressed.warnings = cua_warnings + pressed.warnings
                return pressed
        peekaboo = self._peekaboo_status()
        if peekaboo.get("available"):
            if resolved_engine == "peekaboo" and resolved_peekaboo_snapshot_id and resolved_peekaboo_element_id:
                ...
                if result.returncode == 0:
                    return CommandResult(
                        ...
                        warnings=cua_warnings + self._stderr_warning(result),
                        provenance={"source": "peekaboo"},
                    )
            if target_label:
                result = self.runner([str(peekaboo["path"]), "click", target_label, "--json", "--no-remote"], 15.0)
                if result.returncode == 0:
                    return CommandResult(ok=True, data={"clicked": True, "target_label": target_label, "snapshot_id": snapshot_id, "element_id": element_id, "engine": "peekaboo"}, warnings=cua_warnings + self._stderr_warning(result), provenance={"source": "peekaboo"})
            elif x is not None and y is not None:
                result = self.runner([str(peekaboo["path"]), "click", "--coords", f"{x},{y}", "--global-coords", "--json", "--no-remote"], 15.0)
                if result.returncode == 0:
                    return CommandResult(ok=True, data={"clicked": True, "target_label": resolved_label, "snapshot_id": snapshot_id, "element_id": element_id, "point": self._point(x, y), "engine": "peekaboo"}, warnings=cua_warnings + self._stderr_warning(result), provenance={"source": "peekaboo"})
        if target_label:
        ...
        result = self._mouse_action("click", x=x, y=y, target=resolved_ax_target, require_target=require_snapshot_target)
        result.warnings = cua_warnings + result.warnings
        return result
    ...
    def desktop_type(self, *, text: str, dry_run: bool = False) -> CommandResult:
        ...
        client = self._cua_driver_action_client()
        if client is not None:
            pid, reason = self._cua_driver_frontmost_pid()
            if pid is not None:
                cua_result = client.call(
                    "type_text",
                    {"pid": pid, "text": text, "delivery_mode": "background", "session": "evaos-desktop-bridge"},
                    timeout=20.0,
                )
                acceptable, reason = self._cua_driver_result_acceptable(cua_result, tool="type_text")
                if acceptable:
                    return CommandResult(
                        ok=True,
                        data={
            ...
            cua_warnings.extend(self._cua_driver_warning(reason or "cua-driver type_text failed"))
        peekaboo = self._peekaboo_status()
        if peekaboo.get("available"):
        ...
        result = self._keystroke_arbitrary_text(text)
        result.warnings = cua_warnings + result.warnings
        return result
    ...
    def desktop_scroll(self, *, direction: str = "down", amount: int = 600, dry_run: bool = False) -> CommandResult:
        ...
        if client is not None:
            pid, reason = self._cua_driver_frontmost_pid()
            if pid is not None:
                cua_amount = max(1, min(int(round(amount / 120)) or 1, 50))
                cua_result = client.call(
                    "scroll",
                    {"pid": pid, "direction": direction, "amount": cua_amount, "by": "line", "delivery_mode": "background", "session": "evaos-desktop-bridge"},
                    timeout=10.0,
                )
                acceptable, reason = self._cua_driver_result_acceptable(cua_result, tool="scroll")
                if acceptable:
                    return CommandResult(
                        ok=True,
                        data={"scrolled": True, "direction": direction, "amount": amount, "engine": "cua_driver", "cua_amount": cua_amount},
                        warnings=cua_result.warnings,
                        provenance={"source": "cua_driver"},
                    )
            cua_warnings.extend(self._cua_driver_warning(reason or "cua-driver scroll failed"))
        peekaboo = self._peekaboo_status()
        if peekaboo.get("available"):
            result = self.runner([str(peekaboo["path"]), "scroll", "--direction", direction, "--amount", str(amount), "--json"], 10.0)
            if result.returncode == 0:
                return CommandResult(ok=True, data={"scrolled": True, "direction": direction, "amount": amount, "engine": "peekaboo"}, warnings=cua_warnings + self._stderr_warning(result), provenance={"source": "peekaboo"})
        result = self._mouse_action("scroll", direction=direction, amount=amount)
        result.warnings = cua_warnings + result.warnings
        return result
    ...
    def desktop_hotkey(self, *, keys: str, dry_run: bool = False) -> CommandResult:
        ...
        if client is not None:
            ...
            if pid is not None:
                cua_result = client.call(
                    "hotkey",
                    {"pid": pid, "keys": normalized.split("+"), "delivery_mode": "background", "session": "evaos-desktop-bridge"},
                    timeout=10.0,
                )
                acceptable, reason = self._cua_driver_result_acceptable(cua_result, tool="hotkey")
                if acceptable:
                    return CommandResult(
                        ok=True,
                        data={"pressed": True, "keys": normalized, "engine": "cua_driver", "cua": cua_result.data.get("structured") if isinstance(cua_result.data.get("structured"), dict) else {}},
                        warnings=cua_result.warnings,
                        provenance={"source": "cua_driver"},
                    )
            cua_warnings.extend(self._cua_driver_warning(reason or "cua-driver hotkey failed"))
        peekaboo = self._peekaboo_status()
        if peekaboo.get("available"):
            result = self.runner([str(peekaboo["path"]), "hotkey", "--keys", normalized, "--json", "--no-remote"], 10.0)
            if result.returncode == 0:
                return CommandResult(ok=True, data={"pressed": True, "keys": normalized, "engine": "peekaboo"}, warnings=cua_warnings + self._stderr_warning(result), provenance={"source": "peekaboo"})
        result = self._osascript_hotkey(normalized)
        result.warnings = cua_warnings + result.warnings
        return result
    ...
    def _cua_driver_status(self) -> dict[str, Any]:
        actions_enabled = self._cua_driver_actions_enabled()
        if self.cua_driver_client is not None:
            return {
                "preferred": True,
                "mode": "background",
                "protocol": "mcp_stdio",
                "license": "MIT",
                "active_for_actions": actions_enabled,
                "adapter": "mcp_stdio" if actions_enabled else "disabled",
                "available": True,
                "source": "injected",
                "guidance": "cua-driver is the primary background Mac-control engine when available. Peekaboo and built-in fallbacks remain active recovery paths.",
            }
        path: str | None = None
        bundled_candidates = tuple(bundled_bridge_bin_candidates(("cua-driver",)))
        source = "path"
        for candidate in (*bundled_candidates, *CUA_DRIVER_BIN_CANDIDATES):
            if "/" in candidate:
                if Path(candidate).exists():
                    path = candidate
                    source = "bundled" if candidate in bundled_candidates else "path"
                    break
            else:
                found = shutil.which(candidate)
                if found:
        ...
        base = {
            ...
            "protocol": "mcp_stdio",
            "license": "MIT",
            "active_for_actions": False,
            "adapter": "disabled" if not actions_enabled else "mcp_stdio",
            "guidance": "cua-driver is the primary background Mac-control engine when available. Peekaboo and built-in fallbacks remain active recovery paths.",
        }
        if not path:
            return {
                **base,
                "available": False,
                "install": "Install cua-driver from trycua/cua and approve the required macOS permissions.",
            }
        result = self.runner([path, "--version"], 3.0)
        version = result.stdout.strip() or result.stderr.strip() or None
        return {
            **base,
            ...
            "active_for_actions": bool(actions_enabled and result.returncode == 0),
            "adapter": "mcp_stdio" if actions_enabled and result.returncode == 0 else "disabled",
            "source": source,
            "path": path,
            "version": redact_value(version),
        }

    def _cua_driver_actions_enabled(self) -> bool:
        value = os.environ.get("EVAOS_DESKTOP_BRIDGE_CUA_ACTIONS", "1").strip().lower()
        return value not in {"0", "false", "off", "disabled", "no"}

    def _cua_driver_action_client(self) -> CuaDriverClient | None:
        if not self._cua_driver_actions_enabled():
            return None
        if self.cua_driver_client is not None:
            return self.cua_driver_client
        status = self._cached_engine_probe("cua_driver", self._cua_driver_status)
        path = status.get("path")
        if status.get("available") is True and isinstance(path, str) and path:
            self.cua_driver_client = CuaDriverMcpClient(path)
            return self.cua_driver_client
        return None

    def _cached_engine_probe(self, key: str, probe: Callable[[], dict[str, Any]], *, ttl_seconds: float = 10.0) -> dict[str, Any]:
        now = time.monotonic()
        cached = self._engine_status_cache.get(key)
        if cached is not None and now - cached[0] <= ttl_seconds:
            return dict(cached[1])
        status = probe()
        self._engine_status_cache[key] = (now, dict(status))
        return status

    def _control_engine_status(self) -> dict[str, Any]:
        cua_driver = self._cached_engine_probe("cua_driver", self._cua_driver_status)
        peekaboo = self._cached_engine_probe("peekaboo", self._peekaboo_status)
        if cua_driver.get("available") and cua_driver.get("active_for_actions"):
            active_primary = "cua_driver"
            selection = "cua_driver_primary"
        elif peekaboo.get("available"):
            active_primary = "peekaboo"
            selection = "peekaboo_until_cua_actions_enabled"
        elif self.helper_client is not None:
            active_primary = "post_to_pid_helper"
            selection = "fallback_post_to_pid_helper"
        else:
            active_primary = "accessibility"
            selection = "fallback_accessibility_system_events"
        return {
            "order": list(MAC_CONTROL_ENGINE_ORDER),
            "cua_driver": cua_driver,
            "peekaboo": peekaboo,
            "fallbacks": ["post_to_pid_helper", "accessibility", "system_events"],
    ...
    def _cua_driver_result_acceptable(self, result: CommandResult, *, tool: str, require_verified: bool = False) -> tuple[bool, str | None]:
        if not result.ok:
            message = result.errors[0].get("message") if result.errors else f"cua-driver {tool} failed"
            return False, f"cua-driver {tool} failed: {message}"
        structured = result.data.get("structured") if isinstance(result.data.get("structured"), dict) else {}
        escalation = structured.get("escalation") if isinstance(structured.get("escalation"), dict) else {}
        recommended = str(escalation.get("recommended") or "").strip().lower()
        if recommended == "foreground":
            return False, f"cua-driver requested foreground escalation for {tool}"
        if structured.get("degraded") is True or structured.get("off_space") is True:
            return False, f"cua-driver returned degraded {tool} state"
        if require_verified and structured.get("verified") is not True:
            return False, f"cua-driver {tool} was not verified"
        return True, None

    def _cua_driver_warning(self, reason: str | None) -> list[str]:
        return [reason] if reason else []

    def _cua_driver_target_from_resolved(self, resolved: CommandResult) -> dict[str, Any] | None:
        target = resolved.data.get("cua_target") if isinstance(resolved.data.get("cua_target"), dict) else None
        ...
        try:
            args["window_id"] = int(target["window_id"])
        except (KeyError, TypeError, ValueError):
            pass
        ...
        try:
            args["element_index"] = int(target["element_index"])
        except (KeyError, TypeError, ValueError):
            pass
        if "element_token" not in args and "element_index" not in args:
            return None
        return args

    def _cua_driver_frontmost_pid(self) -> tuple[int | None, str | None]:
        frontmost = self._frontmost_app()
        if not frontmost:
            return None, "cua-driver target app could not be resolved"
        if self._is_sensitive_app(frontmost):
            return None, f"cua-driver blocked for sensitive app {frontmost}"
        pid = self._pid_for_app(frontmost)
        if pid is None:
            return None, "cua-driver target pid could not be resolved"
        return pid, None

    def _cua_driver_see(self, *, client: CuaDriverClient, max_nodes: int) -> CommandResult:
        windows_result = client.call("list_windows", {"on_screen_only": True}, timeout=10.0)
        acceptable, reason = self._cua_driver_result_acceptable(windows_result, tool="list_windows")
        if not acceptable:
            return CommandResult(
                ok=False,
                data={"engine": "cua_driver"},
                warnings=self._cua_driver_warning(reason),
                errors=windows_result.errors,
                provenance={"source": "cua_driver", "tool": "list_windows"},
            )
        structured = windows_result.data.get("structured") if isinstance(windows_result.data.get("structured"), dict) else {}
        windows = structured.get("windows") if isinstance(structured.get("windows"), list) else []
        visible_windows: list[dict[str, Any]] = []
        for window in windows:
            if not isinstance(window, dict):
                continue
            if window.get("is_on_screen") is False:
                continue
            try:
                int(window["pid"])
        ...
        if not visible_windows:
            return CommandResult(
                ok=False,
                data={"engine": "cua_driver"},
                warnings=["cua-driver returned no visible windows"],
                errors=[make_error(code="cua_driver_no_visible_windows", message="cua-driver returned no visible windows.", guidance="Falling back to the next Mac-control engine.")],
                provenance={"source": "cua_driver", "tool": "list_windows"},
            )
        visible_windows.sort(key=lambda item: int(item.get("z_index") or 0), reverse=True)
        window = visible_windows[0]
        pid = int(window["pid"])
        window_id = int(window["window_id"])
        state_result = client.call(
            "get_window_state",
            {
                "pid": pid,
                "window_id": window_id,
                ...
                "session": "evaos-desktop-bridge",
            },
            timeout=15.0,
        )
        acceptable, reason = self._cua_driver_result_acceptable(state_result, tool="get_window_state")
        if not acceptable:
            return CommandResult(
                ok=False,
                data={"engine": "cua_driver", "pid": pid, "window_id": window_id},
                warnings=self._cua_driver_warning(reason),
                errors=state_result.errors,
                provenance={"source": "cua_driver", "tool": "get_window_state"},
            )
        state = state_result.data.get("structured") if isinstance(state_result.data.get("structured"), dict) else {}
        snapshot_id = f"snap-desktop-{uuid.uuid4().hex}"
        elements = self._elements_from_cua(state.get("elements"), snapshot_id=snapshot_id, pid=pid, window_id=window_id, max_nodes=max_nodes)
        if not elements:
            return CommandResult(
                ok=False,
                data={"engine": "cua_driver", "pid": pid, "window_id": window_id, "snapshot_id": snapshot_id},
                warnings=["cua-driver returned no actionable desktop elements"],
                errors=[make_error(code="cua_driver_no_elements", message="cua-driver returned no actionable desktop elements.", guidance="Falling back to the next Mac-control engine.")],
                provenance={"source": "cua_driver", "tool": "get_window_state"},
            )
        self._write_snapshot_index(snapshot_id=snapshot_id, target="desktop", elements=elements, engine="cua_driver")
        return CommandResult(
            ok=True,
            data={
                "engine": "cua_driver",
                "frontmost_app": redact_value(window.get("app_name") or self._frontmost_app()),
                "snapshot_id": snapshot_id,
                "cua_window": {
                    "pid": pid,
                    "window_id": window_id,
                    "app_name": redact_value(window.get("app_name")),
                    "title": redact_value(window.get("title")),
                },
                "elements": elements,
            },
            warnings=windows_result.warnings + state_result.warnings,
            provenance={"source": "cua_driver"},
        )
...
""".strip()
    ...
    def _elements_from_cua(self, nodes: Any, *, snapshot_id: str, pid: int, window_id: int, max_nodes: int) -> list[dict[str, Any]]:
        elements: list[dict[str, Any]] = []
        if not isinstance(nodes, list):
            return elements
        for fallback_index, item in enumerate(nodes):
            ...
            if not isinstance(frame, dict):
                continue
            try:
                x = int(round(float(frame.get("x"))))
                y = int(round(float(frame.get("y"))))
                width = int(round(float(frame.get("w", frame.get("width")))))
                height = int(round(float(frame.get("h", frame.get("height")))))
            except (TypeError, ValueError):
                continue
            if width <= 0 or height <= 0:
                continue
            try:
                element_index = int(item.get("element_index"))
            except (TypeError, ValueError):
                element_index = fallback_index + 1
            token = item.get("element_token")
            label = item.get("label") or item.get("title") or item.get("description") or item.get("role") or f"Element {element_index}"
            label_text, _ = cap_text(str(redact_value(label)), 160)
            role_text, _ = cap_text(str(redact_value(item.get("role") or "unknown")), 80)
            actions: list[str] = []
            if item.get("disabled") is not True:
                actions.append("click")
            cua_target: dict[str, Any] = {
                "pid": int(pid),
                "window_id": int(window_id),
                "element_index": element_index,
            }
            if isinstance(token, str) and token:
                cua_target["element_token"] = token
            elements.append(
                {
                    "element_id": f"cua-{element_index:04d}",
                    "snapshot_id": snapshot_id,
                    "label": label_text,
                    "role": role_text,
                    "bounds": {"x": x, "y": y, "width": width, "height": height},
    ...
    def _resolve_snapshot_target(self, *, snapshot_id: str | None, element_id: str | None, target_label: str | None) -> CommandResult:
        ...
        return CommandResult(
            ...
            data={
                ...
                "cua_target": match.get("cua_target") if isinstance(match.get("cua_target"), dict) else None,
