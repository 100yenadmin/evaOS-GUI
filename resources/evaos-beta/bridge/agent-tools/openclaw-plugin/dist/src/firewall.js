...
export function desktopBridgeFirewall(event) {
  ...
  if (SAFE_TOOL_PREFIXES.some((prefix) => toolName.startsWith(prefix))) {
    ...
    if (APPROVAL_GATED_TOOL_PREFIXES.some((prefix) => toolName.startsWith(prefix))) {
      const params = event.args || event.input || event.params || event.parameters || {};
      ...
      return {
        requireApproval: {
          ...
          description: `${toolName} is a live customer Mac named action. Approval is required and the bridge will audit the command.`,
          severity: 'warning',
