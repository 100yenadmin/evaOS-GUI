...
const ROUTE_CHECKS = [
  ...
  },
  {
    ...
    title: 'About',
    proofStage: PROOF_STAGES.SHELL_SMOKE,
    settledMarkers: ['evaOS Workbench', 'Support', 'Build identity', 'Mac release'],
...
const LOCAL_PRODUCT_ROUTE_CHECKS = [
  ...
  },
  {
    ...
    title: 'People & Access',
    proofStage: PROOF_STAGES.PRODUCT_LOADED_STATE,
    settledMarkers: ['People & Access', 'Account access', 'Fixture Owner', 'fixture-audit-people-policy'],
    loadedStateRequiredMarkers: ['people access broker policy', 'member rows', 'policy audit id'],
    action: 'click-load-default-customer',
    isolateRendererState: true,
    expected: [
      'People & Access',
  ...
  },
  {
    ...
    proofStage: PROOF_STAGES.PRODUCT_LOADED_STATE,
    settledMarkers: ['Connected Apps', 'Provider access', 'Google Workspace', 'fixture-audit-providers'],
    loadedStateRequiredMarkers: ['provider profile rows', 'brokered provider grant', 'provider audit id'],
    action: 'click-load-default-customer',
    isolateRendererState: true,
    expected: [
      'Connected Apps',
  ...
  },
  {
    ...
    title: 'Company Brain',
    proofStage: PROOF_STAGES.PRODUCT_LOADED_STATE,
    settledMarkers: ['Company Brain', 'Company Brain directory', 'Northstar Fixture Account'],
    loadedStateRequiredMarkers: ['company brain native directory', 'account 360 brief'],
    action: 'click-load-default-customer',
    isolateRendererState: true,
    expected: [
      'Company Brain',
  ...
  },
  {
    ...
    expected: [
      ...
      'Handoff target: evaos-workbench://native-companion/status',
...
async function resolveMainWindow(app) {
  ...
  while (Date.now() < deadline) {
    for (const page of app.windows()) {
      ...
      if (text.includes('evaOS Workbench') || text.includes('Mission Control')) {
        return page;
      }
