...
| `diff`, `diff2html`                                                                        | diff viewer and renderer message diff UI         | no process/preload/common/script imports | Diff UI stays renderer-bundled.                                                                                        |
...
| `@dnd-kit/*`, `@floating-ui/react`, `classnames`, `dayjs`, `eventemitter3`, `qrcode.react` | renderer UI helpers                              | no process/preload/common/script imports | Small direct dependencies kept out of production raw shipping when used only by renderer.                              |
...
```bash
rg -n "from ['\"](@arco-design/web-react|@icon-park/react|mermaid|react-syntax-highlighter|@monaco-editor/react|@uiw/react-codemirror|diff2html|react-markdown|streamdown|swr)" \
