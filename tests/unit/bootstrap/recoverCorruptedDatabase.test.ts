...
describe('recoverCorruptedDatabaseAfterUserConfirmation', () => {
  ...
  it('rejects generic startup failures that are not recoverable corruption', async () => {
    const deps = makeDeps({
  ...
  it('restarts the backend with one-shot recovery and reloads the main window after confirmation', async () => {
    const deps = makeDeps({
