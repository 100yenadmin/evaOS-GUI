...
WORKBENCH_RUNTIME_SCHEMA = "evaos.desktop_bridge.workbench_runtime.v1"
WORKBENCH_RUNTIME_CONTRACT_VERSION = 1
BRIDGE_RUNTIME_SEMVER_PATTERN = re.compile(
...
def _public_ready_connector_service_status(status: dict[str, object]) -> dict[str, object]:
    ...
    _add_public_connector_evidence(public, status)
    return public
...
def _add_public_connector_evidence(public: dict[str, object], status: dict[str, object]) -> None:
    private_network = _public_private_network_evidence(status.get("private_network"))
    if private_network:
        public["private_network"] = private_network
    bridge_runtime = _public_workbench_runtime_compatibility(status.get("bridge_runtime"))
    if bridge_runtime:
        public["bridge_runtime"] = bridge_runtime


def _public_private_network_evidence(value: object) -> dict[str, bool]:
    ...
    return {key: item for key, item in value.items() if key in allowed and type(item) is bool}


def _public_workbench_runtime_compatibility(value: object) -> dict[str, object]:
    if not isinstance(value, dict):
        return {}
    if value.get("schema") != WORKBENCH_RUNTIME_SCHEMA:
        return {}
    if value.get("contract_version") != WORKBENCH_RUNTIME_CONTRACT_VERSION:
        return {}
    version = value.get("version")
    if not isinstance(version, str) or not version or len(version) > 64:
        return {}
    if version != "unknown" and BRIDGE_RUNTIME_SEMVER_PATTERN.fullmatch(version) is None:
        return {}
    version_compatible = _is_workbench_runtime_version_compatible(version)
    return {
        "schema": WORKBENCH_RUNTIME_SCHEMA,
        "contract_version": WORKBENCH_RUNTIME_CONTRACT_VERSION,
        "version": version,
...
def _tailscale_ip(status_snapshot: object = _TAILSCALE_STATUS_UNSET) -> str | None:
    interface_ip = _active_tailnet_interface_ip()
    if interface_ip:
        return interface_ip

    status_ip = _online_tailscale_status_ip(status_snapshot)
    if status_ip:
        return status_ip
...
def _tailscale_client_installation_state() -> bool | None:
    if sys.platform != "darwin":
        return None
    executable_candidates = (
        *_tailscale_app_cli_candidates(),
        shutil.which("tailscale"),
        "/opt/homebrew/bin/tailscale",
        "/usr/local/bin/tailscale",
    )
    if any(candidate and Path(candidate).is_file() for candidate in executable_candidates):
        return True
    app_candidates = (
        Path("/Applications/Tailscale.app"),
        Path.home() / "Applications/Tailscale.app",
    )
    return any(path.is_dir() for path in app_candidates)


def _tailscale_status_snapshot() -> dict[str, object] | None:
    commands: list[str] = []
    for candidate in (
        *_tailscale_app_cli_candidates(),
        shutil.which("tailscale"),
...
def _tailscale_app_cli_candidates() -> tuple[str, str]:
    return (
...
def _private_network_evidence(status_snapshot: object = _TAILSCALE_STATUS_UNSET) -> dict[str, bool]:
    evidence: dict[str, bool] = {}
    client_installed = _tailscale_client_installation_state()
    if client_installed is not None:
        evidence["client_installed"] = client_installed

    status = _tailscale_status_snapshot() if status_snapshot is _TAILSCALE_STATUS_UNSET else status_snapshot
    if not isinstance(status, dict):
        return evidence
    evidence["client_installed"] = True
    ...
    if isinstance(self_node, dict):
        ...
        elif online is True and backend_state == "Running":
            evidence["online"] = True
        user_id = self_node.get("UserID")
        if user_id not in (None, "", 0) and evidence.get("enrolled") is not False:
            evidence["enrolled"] = True
    return evidence


def _bridge_runtime_version() -> str:
    try:
...
def _workbench_runtime_compatibility(version: str) -> dict[str, object]:
    normalized = version.strip() if isinstance(version, str) else "unknown"
    if not normalized:
        normalized = "unknown"
    version_compatible = _is_workbench_runtime_version_compatible(normalized)
    return {
        "schema": WORKBENCH_RUNTIME_SCHEMA,
        "contract_version": WORKBENCH_RUNTIME_CONTRACT_VERSION,
        "version": normalized[:64],
        "version_compatible": version_compatible,
        "compatible": version_compatible,
    }
...
def _is_workbench_runtime_version_compatible(version: str) -> bool:
    match = BRIDGE_RUNTIME_SEMVER_PATTERN.fullmatch(version)
    if match is None:
        return False
    release = tuple(int(match.group(name)) for name in ("major", "minor", "patch"))
    return release >= MINIMUM_WORKBENCH_RUNTIME_VERSION


def _online_tailscale_status_ip(status_snapshot: object = _TAILSCALE_STATUS_UNSET) -> str | None:
    payload = _tailscale_status_snapshot() if status_snapshot is _TAILSCALE_STATUS_UNSET else status_snapshot
    if not isinstance(payload, dict):
        return None
