...
export function restartApplication(app: RestartableApp): IAppRestartResult {
  if (!app.isPackaged) {
    console.info('[evaOS Workbench] Restart skipped in development mode; manual restart required');
    return {
      restarted: false,
  ...
  console.info('[evaOS Workbench] Relaunching application to apply changes');
  app.relaunch();
  app.exit(0);
  return {
    restarted: true,
