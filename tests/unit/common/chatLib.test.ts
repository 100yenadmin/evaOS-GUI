...
describe('normalizeTextMessageContent', () => {
  ...
  it('preserves ordinary JSON object strings as plain text', () => {
    expect(normalizeTextMessageContent('{"foo":"bar"}')).toEqual({
      content: '{"foo":"bar"}',
    });
    expect(normalizeTextMessageContent('{"content":"return JSON only"}')).toEqual({
      content: '{"content":"return JSON only"}',
    });
  });
