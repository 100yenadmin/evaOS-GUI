...
| Bucket | Upstream commit |    PR | Class           | Subject                                                   | evaOS status                                                                                                                                                                                                            |
| ------ | --------------: | ----: | --------------- | --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.1.19 |       `57aa0d0` | #3317 | already-present | keep recording when streaming fails before it establishes | Equivalent voice/STT fallback present via `19bb2c0c6`; no new import needed.                                                                                                                                            |
| 2.1.19 |       `87f1d14` | #3310 | already-present | actionable server-side OfficeCLI install guidance         | Equivalent evaOS-branded OfficeCLI guidance and tests already present.                                                                                                                                                  |
| 2.1.19 |       `46ecce4` | #3319 | adapted         | keep disabled custom agents visible in settings           | Adapted in low-risk lane with Settings-only managed-agent cache.                                                                                                                                                        |
...
| 2.1.19 |       `b513c80` | #3324 | adapted         | add observed config option selectors                      | PR5 selectively ports conversation-scoped config option DTOs/client/hook and observed-confirmation setters; 404/no route remains capability-absent fallback.                                                            |
...
| 2.1.19 |       `986e3ee` | #3329 | defer           | remove star office ui remnants                            | Deferred from assistant metadata lane: static search found no Creative Studio/Shared Browser overlap, but Star Office still has extension-settings and OpenClaw monitor wiring that needs a dedicated cleanup decision. |
| 2.1.19 |       `de6240a` | #3333 | adapted         | report installation integrity diagnostics                 | Adapted in updater/support lane: multi-dir report logs plus renderer diagnostics, with evaOS secret redaction kept.                                                                                                     |
| 2.1.19 |       `0fa5c34` | #3334 | defer           | support slot-scoped stop controls                         | Team runtime canary; wait for slot stop/handoff/cancel proof.                                                                                                                                                           |
| 2.1.19 |       `6b097f2` |   n/a | adapted         | defer AI copy row until streaming ends                    | Adapted in low-risk lane with DOM coverage.                                                                                                                                                                             |
| 2.1.19 |       `61f6f65` | #3337 | adapted         | merge AI copy-row polish                                  | Covered by adapted `6b097f2`; no separate import.                                                                                                                                                                       |
| 2.1.19 |       `1650096` | #3336 | adapted         | repair assistant cron and guid metadata flows             | Adapted in assistant metadata lane with evaOS assistant/agent curation preserved.                                                                                                                                       |
| 2.1.19 |       `ff752cf` | #3338 | adapted         | prefer assistant avatars in team chats                    | Adapted in assistant metadata lane; preset assistant metadata wins over generic custom-agent rows when a preset id exists.                                                                                              |
...
| 2.1.20 |       `d1cd349` | #2935 | defer           | Codex ACP image preview and download                      | Deferred to guarded ACP artifact PR; upstream path handling can preview arbitrary files without realpath confinement.                                                                                                   |
| 2.1.20 |       `6dca78b` |   n/a | adapted         | remove leftover gap above assistant list                  | Adapted in low-risk lane.                                                                                                                                                                                               |
| 2.1.20 |       `ecb3c2a` | #3344 | adapted         | merge assistant list gap fix                              | Covered by adapted `6dca78b`; no separate import.                                                                                                                                                                       |
...
| 2.1.20 |       `81a74d2` | #3351 | adapted         | add singleton update notification                         | Adapted reliability primitives only: manual download dedupe/cancel, auto-update cancel/restore hooks, current-version status; rejected upstream CDN/stable feed.                                                        |
| 2.1.20 |       `bdceeb4` | #3358 | adapted         | combine header model thinking selector                    | PR5 adapts combined model/thinking selector behind runtime capability detection, with team/scoped sessions preventing global preference persistence.                                                                    |
...
| 2.1.21 |       `b97ed41` | #3361 | adapted         | wire pt-BR into language pickers and main-process loader  | Adapted in low-risk lane; also registered already-visible `uk-UA` in main-process i18n.                                                                                                                                 |
| 2.1.21 |       `12b591b` | #3366 | adapted         | build valid file URL for PDF preview on Windows           | Adapted in low-risk lane with preview tests; Windows proof stays in #305.                                                                                                                                               |
| 2.1.21 |       `d51d493` | #3369 | adapted         | restore local html and selected file reopen               | Adapted behavior without extra renderer debug IPC; clean HTML uses file URL, dirty edits stay in memory.                                                                                                                |
