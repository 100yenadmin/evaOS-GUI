...
1. **Test on the active release profile** before committing. `EVAOS_RELEASE_TARGET_PLATFORMS=all` is the full Windows/macOS/Linux contract; `EVAOS_RELEASE_TARGET_PLATFORMS=macos` is the universal macOS profile; `EVAOS_RELEASE_TARGET_PLATFORMS=macos-arm64` is the controlled Apple-Silicon RC profile that intentionally omits x64, Windows, and Linux assets.
