...
async function providerAgentDiscoveryPayload(
  agentRuntime: 'openclaw' | 'hermes'
): Promise<ProviderDiscoveryPayload | null> {
  const endpoint = providerDiscoveryEndpoint();
  const customerID = process.env.EVAOS_CUSTOMER_ID?.trim();
...
async function cacheProviderGrantFromBroker(
  agentRuntime: 'openclaw' | 'hermes',
  providerKey: string,
  payload: unknown
): Promise<boolean> {
...
async function postRemoteBridgeCommand(
  endpoint: URL,
